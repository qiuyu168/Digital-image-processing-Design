# 本文件用于手动测试单个后端图像处理算法，只需要修改三项配置即可运行。
...
# 3. 要进行本地测试的算法导入路径
# 示例：
# app.algorithms.grayscale_image.grayscale
# app.algorithms.grayscale_image.binary_threshold
# app.algorithms.color_image.saturation_adjust
# app.algorithms.geometric_transform.rotate
# app.algorithms.spatial_filter.gaussian_filter
# app.algorithms.frequency_analysis.dft_spectrum
# app.algorithms.frequency_filter.low_pass_filter
...
def main() -> None:
    ...
    print("=" * 68)
    print(f"要处理的图片：{INPUT_IMAGE_PATH}")
    print(f"处理后的图片：{OUTPUT_IMAGE_PATH}")
    print(f"测试算法路径：{ALGORITHM_IMPORT_PATH}")

    run_algorithm, algorithm_meta = load_algorithm(ALGORITHM_IMPORT_PATH)

    params = build_default_params(algorithm_meta)
    print(f"自动使用默认参数：{json.dumps(params, ensure_ascii=False)}")
...
def load_algorithm(import_path: str):
    ...
    try:
        ...
        raise ModuleNotFoundError(
            ...
            f"请检查 ALGORITHM_IMPORT_PATH 是否写对，并确认当前命令在 backend 目录下运行。"
        ) from exc
...
def build_default_params(algorithm_meta: dict[str, Any]) -> dict[str, Any]:
    """根据 ALGORITHM_META 自动生成默认参数，避免使用者手动修改参数。"""
...
def read_image_unicode(path: str | Path) -> np.ndarray:
    ...
    if image is None:
        raise ValueError(f"图片读取失败，请检查图片格式是否正确：{image_path}")
