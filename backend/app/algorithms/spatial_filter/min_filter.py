...
ALGORITHM_META = {
    ...
    "name": "min_filter",
    "display_name": "最小值滤波",
    "description": "用邻域最小值替代中心像素，增强暗区域并收缩高亮结构。",
    "params": {
...
def run(image: np.ndarray, params: dict | None = None) -> dict:
    ...
    result = cv2.erode(source, np.ones((kernel_size, kernel_size), dtype=np.uint8)).astype(np.uint8)
    return {
        "result": result,
        "steps": [{"name": "原始图像", "image": source}, {"name": "处理结果", "image": result}],
        "metrics": {"kernel_size": kernel_size, **gray_metrics(source, result)},
        "analysis": "最小值滤波输出邻域内最小灰度，能扩张暗区域并抑制孤立亮点噪声。",
    }
